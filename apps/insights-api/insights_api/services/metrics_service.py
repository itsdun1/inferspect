...
async def latency(client: Any, *, window: str, group: str) -> dict[str, Any]:
    group = _validate_group(group)
    since = since_for(window)
    rows = await repo.latency_buckets(client, since=since, group=group)
    return {"window": window, "group": group, "buckets": rows}


async def throughput(client: Any, *, window: str, group: str) -> dict[str, Any]:
    group = _validate_group(group)
    since = since_for(window)
    rows = await repo.throughput_buckets(client, since=since, group=group)
...
async def errors(client: Any, *, window: str, sample_size: int = 5) -> dict[str, Any]:
    since = since_for(window)
    rows = await repo.error_counts(client, since=since, sample_size=sample_size)
    return {"window": window, "groups": rows}


async def cost(client: Any, *, window: str, group: str) -> dict[str, Any]:
    group = _validate_group(group)
    since = since_for(window)
    by_group = await repo.cost_by_group(client, since=since, group=group)
    top_convos = await repo.top_cost_conversations(client, since=since, limit=10)
    return {
        "window": window,
        "group": group,
        "by_group": by_group,
...
async def top_conversations(
    client: Any, *, metric: str, limit: int, window: str = "24h"
) -> dict[str, Any]:
    ...
    rows = await repo.top_conversations(
        client, since=since, metric=metric, limit=limit
    )
    return {"window": window, "metric": metric, "limit": limit, "conversations": rows}


async def summary(client: Any, *, window: str) -> dict[str, Any]:
    ...
    """
    since = since_for(window)
    row = await repo.summary_rollup(client, since=since)
