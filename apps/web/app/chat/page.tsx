...
export default function ChatPage() {
  ...
  // we still fetch the user record so we can show the email + admin link.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
  ...
  async function handleCancel() {
    // 1. Tell the backend to cancel the in-flight task (it will fire the
    //    StreamRegistry, persist a 'cancelled' assistant message, and emit
    //    the 'cancelled' SSE event).
    // 2. Abort the client-side fetch so we stop consuming the stream.
    if (activeId) {
      try {
