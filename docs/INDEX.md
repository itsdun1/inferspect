...
- [research/straiker-deep-dive.md](./research/straiker-deep-dive.md) — full competitive deep dive on Straiker (the leading agentic-AI-security startup): company/funding/team, the 3-product platform (Discover/Ascend/Defend), technical architecture, open-source-vs-from-scratch verdict (plumbing = OpenTelemetry + Kong/Portkey gateways; detection = proprietary fine-tuned model cloud; eBPF = observe-only), and where we're differentiated (in-kernel enforcement, on-host PII, content-identity, insurance) vs behind (detection ML, red-teaming).
- [research/repello-deep-dive.md](./research/repello-deep-dive.md) — full competitive deep dive on Repello AI (seed-stage, IIT-Roorkee founders, $1.2M): the discover→test→runtime pipeline (Inventory / ARTEMIS red-teaming / ARGUS guardrails / MCP Gateway), app-layer SDK+proxy architecture (no eBPF/kernel), open-source verdict (thin OSS clients + frontier-LLM orchestration; closed proprietary engine; their public OSS = Agent-Wiz, whistleblower), their "BREACHED" multi-turn taxonomy worth borrowing, and a Repello-vs-Straiker table. Same two gaps as Straiker: no in-kernel enforcement, no insurance.
